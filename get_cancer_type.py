...
con_matrix = pd.read_csv("consequence_by_tumor.csv", index_col=0)
...
annotated_con_matrix = con_matrix.copy()
annotated_con_matrix["Cancer_Type"] = annotated_con_matrix.index.str.slice(0, 12).map(patient_to_acronym)

# save annotated matrix
annotated_matrix.to_csv("sbs96_by_tumor_with_cancer_type.csv")
annotated_con_matrix.to_csv("consequence_by_tumor_with_cancer_type.csv")

#print cancer types
print(annotated_matrix["Cancer_Type"].value_counts())